...
version=3.8 #version (For IDE and more)
...
try:
    ...
    class Interpreter:
        ...
        def cmd_host_file(self, args):
            """
            Hosts a folder as a live-reloading HTTP server.
            Syntax: host_file <directory> <port> [--interval=SECONDS]
            Options:
                --interval=SECONDS  Set live-reload check interval (default: 1)
            """
            import http.server
            import socketserver
            import threading
            ...
            try:
                # Parse arguments with argparse for better handling
                        ...
                        try:
                            with open(index_path, "r+", encoding='utf-8') as f:
                                ...
                                if "<!-- live reload -->" not in content:
                                    ...
                                    """
                                    # Insert before closing body or head tag if found
                                    if "</body>" in content:
                                    ...
                                    f.write(content)
                                    f.truncate()
                            break  # Only inject into the first found index file
                ...
                class ReloadHandler(FileSystemEventHandler):
                    def on_modified(self, event):
                        if not event.is_directory and event.src_path != str(reload_file):
                            with open(reload_file, "w") as f:
                                f.write(str(time.time()))  # change value to trigger reload
                ...
                class SafeHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
                    ...
                    def log_message(self, format, *args):
                            return  # Silence reload polling logs
                observer = Observer()
                ...
                try:
                    ...
                    if reload_file.exists():
                        reload_file.unlink()  # Clean up reload file
